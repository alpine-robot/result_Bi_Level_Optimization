...
FOLDERS = json.loads(plot_str) if plot_str else [
    "up_rocky/exploration/run_1",
    "up_rocky/exploration/run_2",
    "up_rocky/exploration/run_3",
    "up_rocky/exploration/run_4",
    "up_rocky/exploration/run_5",
    "up_rocky/exploration/run_6",
    "up_rocky/exploration/run_7",
    "up_rocky/exploration/run_8",
    "up_rocky/exploration/run_9",
    "up_rocky/exploration/run_10",
    ]
...
class CombinePlot:
    ...
    def plot_combined_fitness_jumps_traj(self):
        """Single figure with 3 subplots: fitness by iteration, jump histogram, 3D trajectory."""
        if not self.results:
            return

        fig = plt.figure(figsize=(30, 8))
        ax1 = fig.add_subplot(1, 3, 1)          # fitness
        ax2 = fig.add_subplot(1, 3, 2)          # jump histogram
        ax3 = fig.add_subplot(1, 3, 3, projection='3d')  # trajectory

        # ── 1) FITNESS BY ITERATION (same logic as plot_fitness_by_iteration_2) ──
        all_iter_bests, all_global_bests, all_elite_fits = [], [], []
        max_len = 0
        ...
        for k, r in enumerate(self.results):
            ...
            ax1.scatter(iters[~best_mask], fits[~best_mask],
                        color=c, s=30, alpha=0.6, edgecolors='black')
            if any(best_mask):
                ax1.scatter(iters[best_mask], fits[best_mask],
                            color=c, s=60, marker='*', edgecolors='red', zorder=4)
        ...
        ax1.plot(x_axis, med_elite, color='green', lw=2.5, zorder=5, label='Median Elite Fitness')
        ax1.fill_between(x_axis, p25_elite, p75_elite, color='green', alpha=0.2, zorder=4,
                         label='25th-75th Percentile')

        mat_global = pad(all_global_bests, max_len)
        med_global = np.nanmedian(mat_global, axis=0)
        ax1.plot(x_axis, med_global, color='red', lw=2.5, zorder=5,
                 marker='*', markersize=6, label='Median Best (Overall)')

        # ax1.set_title("Elite Fitness by Iteration", fontsize=16, fontweight='bold')
        ...
        if any(all_jumps):
            ...
            for k, (jumps, r) in enumerate(zip(all_jumps, self.results)):
                counts = [jumps.count(v) for v in vals]
                ax2.bar(vals + (k - len(self.results) / 2 + 0.5) * bw, counts, width=bw,
                        color=self.cmap(k), edgecolor="black", alpha=0.8, label=r.label)

            # ax2.set_title("Jump Count Distribution", fontsize=16, fontweight='bold')
            ax2.set_xlabel("Number of Jumps", fontsize=14)
        ...
        pts = np.array([p["position"] for p in ref.points_t_data])
        costs = np.array([p["cost"] for p in ref.points_t_data])
        ax3.scatter(*pts.T, c=costs, cmap="RdYlGn_r", s=10, alpha=0.05, zorder=1)
        ...
        for k, r in enumerate(self.results):
            ...
            for i, seg in enumerate(r.best_traj_ever):
                s = np.array(seg)
                xs, ys, zs = s if s.shape[0] == 3 and s.shape[1] != 3 else s.T
                ax3.plot(xs, ys, zs, color=draw_color, lw=traj_lw, alpha=traj_alpha, zorder=zorder_traj)
                all_x.append(xs); all_y.append(ys); all_z.append(zs)
                landing_points.append([xs[0], ys[0], zs[0]])
                if i == len(r.best_traj_ever) - 1:
                    landing_points.append([xs[-1], ys[-1], zs[-1]])
            ...
            lp = np.array(landing_points)
            ax3.scatter(lp[:, 0], lp[:, 1], lp[:, 2],
                        c='yellow', s=70 if is_best else 30,
                        edgecolors='black', alpha=scatter_alpha, zorder=zorder_traj + 1)
            ax3.scatter(*r.p0, c='lime', s=200, marker="^",
                        edgecolors="k", linewidth=2, alpha=scatter_alpha, zorder=15)
            ax3.scatter(*r.pf, c="red", s=200, marker="X",
                        edgecolors="k", linewidth=2, alpha=scatter_alpha, zorder=15)

            if r.best_achieved_target_ever:
                ach = np.array(r.best_achieved_target_ever).flatten()
                ax3.scatter(*ach, c="orange", s=250, marker="*",
                            edgecolors="k", alpha=scatter_alpha, zorder=16)
                ax3.plot(*zip(r.pf, ach), "--", color=draw_color, lw=2,
        ...
        ax3.zaxis.set_major_locator(plt.MaxNLocator(2))
        ax3.set_ylabel('Y [m]', fontsize=14)
        ax3.set_zlabel('Z [m]', fontsize=14)
        ax3.tick_params(axis='y', labelsize=12)
        ax3.tick_params(axis='z', labelsize=12)
        # ax3.set_title("Best Trajectories", fontsize=16, fontweight='bold')
        ax3.view_init(elev=20, azim=-20)

        fig.tight_layout()
        _save_plot(fig, "combined_fitness_jumps_traj")
        plt.show()

if __name__ == "__main__":
    cp = CombinePlot(FOLDERS)
    if cp.results:
        # cp.plot_combined_fitness_jumps_traj()
