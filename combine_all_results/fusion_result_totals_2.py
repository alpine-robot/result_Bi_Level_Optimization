...
FUSION_GROUPS='[{"name":"Gaussian (legacy)","path":"up_gaussian","terrain":"gaussian","variant":"legacy"},
                {"name":"Gaussian (05)","path":"result_05","run_glob":"test_05_*_gaussian_bumps","terrain":"gaussian","variant":"05"}]' \
python3 fusion_result.py
"""
...
if _env:
    ...
    GROUPS = [
        # Legacy layout: up_<terrain>/run_<N>
        {"name": "Gaussian (legacy)", "path": "up_gaussian", "terrain": "gaussian", "variant": "legacy"},
        {"name": "Rocky (legacy)", "path": "up_rocky", "terrain": "rock", "variant": "legacy"},
        {"name": "Hemisphere (legacy)", "path": "up_hemisphere", "terrain": "hemisphere", "variant": "legacy"},

        # New layout: result_05/test_05_<run>_<terrain>
        {"name": "Gaussian (05)", "path": "result_05", "run_glob": "test_05_*_gaussian_bumps", "terrain": "gaussian", "variant": "05"},
        {"name": "Rocky (05)", "path": "result_05", "run_glob": "test_05_*_rock", "terrain": "rock", "variant": "05"},
        {"name": "Hemisphere (05)", "path": "result_05", "run_glob": "test_05_*_hemisphere", "terrain": "hemisphere", "variant": "05"},
    ]

OUTPUT_DIR = "result_fusion_totals_2"
...
# Helpers
# ──────────────────────────────────────────────
def discover_runs(group: dict) -> list[str]:
    """Return sorted list of run folders for one group."""
    group_path = group["path"]
    run_glob = group.get("run_glob", "run_*")
    pattern = os.path.join(group_path, run_glob)

    def sort_key(path: str):
        # Sort by first number in folder name when available, then by name.
        name = os.path.basename(path)
        m = re.search(r"\d+", name)
        return (int(m.group()), name) if m else (10**9, name)
...
def global_best_matrix(results: list[SingleResult], max_iters: int = 10) -> np.ndarray:
    """
    ...
    for r in results:
        ...
        series = []
        for it_elites in r.all_elites[:max_iters]:
    ...
    mat = np.full((len(series_list), max_iters), np.nan)
    for i, s in enumerate(series_list):
        if not s:
            continue

        end = min(len(s), max_iters)
        mat[i, :end] = s[:end]
        if end < max_iters:
            mat[i, end:] = s[end - 1]
    return mat
...
def get_group_style(group: dict, idx: int) -> dict:
    """Return plotting style for a group, using terrain + variant metadata."""
    terrain = group.get("terrain")
    color = TERRAIN_COLORS.get(terrain, COLORS[idx % len(COLORS)])
    is_05 = str(group.get("variant", "")).strip() == "05"
    return {
        "color": color,
        "line_style": "--" if is_05 else "-",
        "hatch": "///" if is_05 else None,
        "fill_alpha": 0.14 if is_05 else 0.22,
    }
...
def plot_jump_histogram_fusion(groups: list[dict]):
    ...
    for k, (jumps, name) in enumerate(zip(group_jumps, group_names)):
        ...
        style = get_group_style(groups[k], k)
        ax.bar(
...
def plot_median_best_overall(groups: list[dict]):
    ...
    for idx, group in enumerate(groups):
        ...
        mat = global_best_matrix(results, max_iters=10)          # (n_runs × 10)
        x = np.arange(1, 11)
        ...
        style = get_group_style(group, idx)
        n_runs = mat.shape[0]
        ...
        ax.fill_between(x, q1, q3, color=style["color"], alpha=style["fill_alpha"])
...
def plot_convergence_fusion(groups: list[dict]):
    ...
    for idx, (group, entry) in enumerate(zip(groups, group_rate_matrices)):
        ...
        ax1.bar(xs, med_cv, bw, color=style["color"], edgecolor="black",
            hatch=style["hatch"],
                label=group["name"], alpha=0.85)
        ax1.bar(xs, med_fl, bw, bottom=med_cv, color=style["color"],
    ...
    for idx, (group, entry) in enumerate(zip(groups, group_rate_matrices)):
        ...
        ax2.plot(x, med_rate, color=style["color"], lw=2.5,
             ls=style["line_style"],
                 marker="o", markersize=4, label=group["name"])
        ax2.fill_between(x, q1_rate, q3_rate, color=style["color"], alpha=style["fill_alpha"])
